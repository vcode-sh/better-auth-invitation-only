...
describe("validateInviteCode", () => {
	...
	it("returns valid=false for revoked invitation", async () => {
		...
		adapter.findOne.mockResolvedValue(makeInvitation({ revokedAt: new Date() }));
		const result = await handler(ctx);
		expect(result.valid).toBe(false);
	});
	...
	it("SECURITY: does NOT return email field (PII leak prevention)", async () => {
		const { adapter, ctx } = makeCtx({ body: { code: "valid-code" } });
		adapter.findOne.mockResolvedValue(makeInvitation({ email: "secret@private.com" }));
		const result = await handler(ctx);
	...
	it("SECURITY: does NOT return email even for invalid codes", async () => {
		...
		adapter.findOne.mockResolvedValue(makeInvitation({ usedAt: new Date(), email: "leak@test.com" }));
		const result = await handler(ctx);
		expect(result).not.toHaveProperty("email");
	});
	...
	it("SECURITY: handles XSS payload in code without crashing", async () => {
		const { adapter, ctx } = makeCtx({ body: { code: '<script>alert("xss")</script>' } });
		adapter.findOne.mockResolvedValue(null);
		const result = await handler(ctx);
		expect(result.valid).toBe(false);
	});
	...
	it("SECURITY: handles SQL injection payload in code", async () => {
		const { adapter, ctx } = makeCtx({ body: { code: "'; DROP TABLE invitation; --" } });
		adapter.findOne.mockResolvedValue(null);
		const result = await handler(ctx);
		expect(result.valid).toBe(false);
	...
	it("calls ctx.json with the response data", async () => {
		...
		adapter.findOne.mockResolvedValue(null);
		await handler(ctx);
		expect(ctx.json).toHaveBeenCalledWith({ valid: false, expiresAt: undefined });
	});
});
...
describe("getInviteConfig", () => {
	...
	it("propagates error if enabled function throws", async () => {
		const endpoints = createPublicEndpoints({
			enabled: () => { throw new Error("config DB down"); },
		});
		const { ctx } = makeCtx();
		await expect(endpoints.getInviteConfig(ctx)).rejects.toThrow("config DB down");
	});

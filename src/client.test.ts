...
describe("inviteOnlyClient", () => {
  ...
  it("getActions returns all expected methods", () => {
    ...
    expect(actions.createBatchInvitations).toBeTypeOf("function");
    expect(actions.deleteInvitation).toBeTypeOf("function");
...
describe("createInvitation", () => {
  ...
  it("passes maxUses and metadata through", async () => {
    const mockFetch = createMockFetch({
      "/invite-only/create": { id: "1", code: "abc", email: "a@b.com" },
    });
    const actions = inviteOnlyClient().getActions(mockFetch, {}, {});
    ...
    expect(mockFetch).toHaveBeenCalledWith("/invite-only/create", {
      method: "POST",
      body: { email: "a@b.com", maxUses: 5, metadata: { team: "engineering" } },
    });
  });
});

describe("createBatchInvitations", () => {
  it("calls POST /invite-only/create-batch with correct body", async () => {
    const mockFetch = createMockFetch({
      "/invite-only/create-batch": { created: 2, invitations: [] },
    });
    const actions = inviteOnlyClient().getActions(mockFetch, {}, {});

    const invitations = [
      { email: "a@b.com", sendEmail: true },
      { email: "c@d.com", maxUses: 3, metadata: { role: "dev" } },
    ];

    const result = await actions.createBatchInvitations({ invitations });

    expect(mockFetch).toHaveBeenCalledWith("/invite-only/create-batch", {
      method: "POST",
      body: { invitations },
    });
...
describe("deleteInvitation", () => {
  it("calls POST /invite-only/delete with id", async () => {
    const mockFetch = createMockFetch({
      "/invite-only/delete": { success: true },
    });
    const actions = inviteOnlyClient().getActions(mockFetch, {}, {});

    const result = await actions.deleteInvitation({ id: "inv-5" });

    expect(mockFetch).toHaveBeenCalledWith("/invite-only/delete", {
      method: "POST",
      body: { id: "inv-5" },
    });
...
describe("listInvitations", () => {
  it("calls GET /invite-only/list with no params by default", async () => {
    const mockFetch = createMockFetch({
      "/invite-only/list": { items: [], nextCursor: undefined },
    });
    const actions = inviteOnlyClient().getActions(mockFetch, {}, {});

    await actions.listInvitations();

    expect(mockFetch).toHaveBeenCalledWith("/invite-only/list", {
      method: "GET",
    });
  });
...
describe("revokeInvitation", () => {
  it("calls POST /invite-only/revoke with id", async () => {
    const mockFetch = createMockFetch({
      "/invite-only/revoke": { success: true },
    });
    const actions = inviteOnlyClient().getActions(mockFetch, {}, {});
...
describe("resendInvitation", () => {
  it("calls POST /invite-only/resend with id", async () => {
    const mockFetch = createMockFetch({
      "/invite-only/resend": {
        success: true,
        newInvitationId: "inv-3",
        inviteUrl: "https://app.com/register?invite=newcode",
      },
    });
    const actions = inviteOnlyClient().getActions(mockFetch, {}, {});
...
describe("validateInviteCode", () => {
  ...
  it("returns valid: false for invalid code", async () => {
    const mockFetch = createMockFetch({
      "/invite-only/validate": { valid: false },
    });
    const actions = inviteOnlyClient().getActions(mockFetch, {}, {});
...
describe("getInviteConfig", () => {
  it("calls GET /invite-only/config and returns only enabled", async () => {
    const mockFetch = createMockFetch({
      "/invite-only/config": { enabled: true },
    });
    const actions = inviteOnlyClient().getActions(mockFetch, {}, {});

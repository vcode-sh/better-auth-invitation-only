...
describe("parseInviteCodeFromCookie - Security", () => {
  ...
  it("parses normal cookie and returns undefined for missing", () => {
    expect(parseInviteCodeFromCookie("ba-invite-code=abc; o=v", NAME)).toBe(
      "abc"
    );
  ...
  it("decodes percent-encoded values including null bytes", () => {
    expect(parseInviteCodeFromCookie("ba-invite-code=abc%00def", NAME)).toBe(
      "abc\0def"
    );
  });
  ...
  it("double-encoded values decode only once", () => {
    // %2561 -> %61 (not "a")
    expect(parseInviteCodeFromCookie("ba-invite-code=%2561bc", NAME)).toBe(
      "%61bc"
    );
  });
  ...
  it("malformed percent-encoding falls back to raw value", () => {
    expect(parseInviteCodeFromCookie("ba-invite-code=%GGbad", NAME)).toBe(
      "%GGbad"
    );
  });
  ...
  it("regex special chars in cookie name are escaped properly", () => {
    expect(
      parseInviteCodeFromCookie("ba.invite+code=found", "ba.invite+code")
    ).toBe("found");
  });

  it("does not match partial cookie names (prefix attack)", () => {
    expect(
      parseInviteCodeFromCookie("ba-invite-code-ext=evil", NAME)
    ).toBeUndefined();
  });
  ...
  it("first cookie wins when duplicates exist", () => {
    expect(
      parseInviteCodeFromCookie(
        "ba-invite-code=first; ba-invite-code=second",
        NAME
      )
    ).toBe("first");
  });
...
describe("Input Fuzzing", () => {
  ...
  it("buildInviteUrl encodes special chars in code", () => {
    expect(buildInviteUrl("https://a.com", "/r", "a b&c=d")).toBe(
      "https://a.com/r?invite=a%20b%26c%3Dd"
    );
  });

  it("buildInviteUrl rejects javascript: protocol (XSS prevention)", () => {
    expect(() => buildInviteUrl("javascript:alert(1)", "/r", "c")).toThrow(
      "Invalid base URL protocol"
    );
  });

  it("buildInviteUrl passes path traversal through unvalidated", () => {
    expect(buildInviteUrl("https://a.com", "/../../../etc/passwd", "c")).toBe(
      "https://a.com/../../../etc/passwd?invite=c"
    );
  });

  it("buildInviteUrl strips only one trailing slash", () => {
    expect(buildInviteUrl("https://a.com//", "/r", "c")).toBe(
      "https://a.com//r?invite=c"
    );
  });
...
describe("buildInviteUrl - Security", () => {
  it("encodes invite code and handles missing leading slash", () => {
    expect(buildInviteUrl("https://a.com", "/r", "abc")).toBe(
      "https://a.com/r?invite=abc"
    );
    expect(buildInviteUrl("https://a.com", "r", "abc")).toBe(
      "https://a.com/r?invite=abc"
    );
  ...
  it("data: URI scheme is rejected (XSS prevention)", () => {
    expect(() =>
      buildInviteUrl("data:text/html,<h1>hi</h1>", "/x", "c")
    ).toThrow("Invalid base URL protocol");
  });

  it("encodes all special URL chars in code", () => {
    expect(buildInviteUrl("https://a.com", "/r", "?foo=bar&x=1#hash")).toBe(
      "https://a.com/r?invite=%3Ffoo%3Dbar%26x%3D1%23hash"
    );
  });

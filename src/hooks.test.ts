...
const inv = (o: Record<string, any> = {}) => ({
	id: "inv-1", email: "alice@example.com", codeHash: hashInviteCode("valid-code"),
	invitedBy: "admin-1", usedBy: null, usedAt: null, revokedAt: null,
	expiresAt: future(), createdAt: new Date(), ...o,
});

const mkAdapter = () => ({
	findOne: vi.fn(), findMany: vi.fn(), create: vi.fn(), update: vi.fn(), count: vi.fn(),
});

const OPTS = {
	enabled: true as const, emailSignupPath: "/sign-up/email",
	interceptOauth: true, oauthPrefix: "/callback/", cookieName: "ba-invite-code",
};

const eCtx = (adapter: any, body: any) => ({
	path: "/sign-up/email", body, headers: new Headers(), context: { adapter },
});

const oCtx = (adapter: any, cookie?: string) => {
	const h = new Headers();
	if (cookie !== undefined) h.set("cookie", `ba-invite-code=${cookie}`);
	return { path: "/callback/google", body: {}, headers: h, context: { adapter } };
};
...
const aCtx = (adapter: any, user: any, cookie?: string) => {
	...
	return {
		path: "/sign-up/email", body: {}, headers: h,
		context: { adapter, newUser: user, logger: { error: vi.fn(), warn: vi.fn() } },
	};
};

describe("hooks.ts", () => {
	beforeEach(() => { __pendingInvites.clear(); vi.restoreAllMocks(); });

	// --- cleanupPendingInvites ---
	describe("cleanup", () => {
		it("removes expired, keeps fresh", () => {
			__pendingInvites.set("old", { invitationId: "x", createdAt: Date.now() - PENDING_TTL_MS - 1 });
		...
		it("removes ALL expired entries (not just first)", () => {
			for (let i = 0; i < 50; i++)
				__pendingInvites.set(`e${i}`, { invitationId: `i${i}`, createdAt: Date.now() - PENDING_TTL_MS - 1 });
			__pendingInvites.set("keep", { invitationId: "k", createdAt: Date.now() });
			cleanupPendingInvites();
		...
		it("entry at exactly TTL boundary is NOT removed (off-by-one)", () => {
			...
			__pendingInvites.set("boundary", { invitationId: "b", createdAt: Date.now() - PENDING_TTL_MS });
			cleanupPendingInvites();
	...
	describe("startCleanupInterval", () => {
		it("idempotent — multiple calls do not throw", () => {
			expect(() => { startCleanupInterval(); startCleanupInterval(); }).not.toThrow();
		});
	});
	...
	describe("before — email signup", () => {
		it("INVITE_REQUIRED when no inviteCode", async () => {
			...
			await expect(h[0].handler(eCtx(a, { email: "a@b.com" }))).rejects.toThrow(ERROR_CODES.INVITE_REQUIRED);
		});

		it("INVALID_INVITE when code not found", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(null);
			await expect(createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "a@b.com", inviteCode: "bad" })))
				.rejects.toThrow(ERROR_CODES.INVALID_INVITE);
		});

		it("INVALID_INVITE for expired invitation", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv({ expiresAt: past() }));
			await expect(createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "a@b.com", inviteCode: "valid-code" })))
				.rejects.toThrow(ERROR_CODES.INVALID_INVITE);
		});

		it("INVALID_INVITE for used invitation", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv({ usedAt: new Date(), usedBy: "x" }));
			await expect(createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "a@b.com", inviteCode: "valid-code" })))
				.rejects.toThrow(ERROR_CODES.INVALID_INVITE);
		});

		it("INVALID_INVITE for revoked invitation", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv({ revokedAt: new Date() }));
			await expect(createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "a@b.com", inviteCode: "valid-code" })))
				.rejects.toThrow(ERROR_CODES.INVALID_INVITE);
		});

		it("stores pending entry keyed by lowercased email", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv());
			await createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "ALICE@Example.COM", inviteCode: "valid-code" }));
			expect(__pendingInvites.has("alice@example.com")).toBe(true);
		});

		it("SEC-6: EMAIL_MISMATCH when signup email != invitation email", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv({ email: "alice@example.com" }));
			await expect(createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "mallory@evil.com", inviteCode: "valid-code" })))
				.rejects.toThrow(ERROR_CODES.EMAIL_MISMATCH);
		});

		it("allows any email when invitation.email is empty (no binding)", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv({ email: "" }));
			await expect(createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "anyone@x.com", inviteCode: "valid-code" })))
				.resolves.not.toThrow();
		});

		it("bypasses gate when enabled=false", async () => {
			const a = mkAdapter();
			await expect(createBeforeHooks({ ...OPTS, enabled: false })[0].handler(eCtx(a, {})))
				.resolves.not.toThrow();
		});

		it("bypasses gate when enabled is async () => false", async () => {
			const a = mkAdapter();
			await expect(createBeforeHooks({ ...OPTS, enabled: async () => false })[0].handler(eCtx(a, {})))
				.resolves.not.toThrow();
		});

		it("enabled defaults to true when undefined", async () => {
			const a = mkAdapter();
			await expect(createBeforeHooks({ ...OPTS, enabled: undefined })[0].handler(eCtx(a, { email: "a@b.com" })))
		...
		it("SEC-2 FIX: map at PENDING_MAX_SIZE with all fresh entries rejects with TOO_MANY_REQUESTS", async () => {
			...
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv());
			await expect(
				createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "alice@example.com", inviteCode: "valid-code" })),
		...
		it("no pending entry when body has no email", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv({ email: "" }));
			await createBeforeHooks(OPTS)[0].handler(eCtx(a, { inviteCode: "valid-code" }));
			expect(__pendingInvites.size).toBe(0);
		});

		it("handles null body gracefully", async () => {
			const a = mkAdapter();
			await expect(createBeforeHooks(OPTS)[0].handler(eCtx(a, null)))
				.rejects.toThrow(ERROR_CODES.INVITE_REQUIRED);
		});

		it("email with only whitespace is treated as falsy (no pending set)", async () => {
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv({ email: "" }));
			await createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "   ", inviteCode: "valid-code" }));
		...
		it("same user signing up twice overwrites pending entry", async () => {
			const a = mkAdapter();
			a.findOne.mockResolvedValueOnce(inv({ id: "inv-A" })).mockResolvedValueOnce(inv({ id: "inv-B" }));
			const h = createBeforeHooks(OPTS);
			await h[0].handler(eCtx(a, { email: "alice@example.com", inviteCode: "valid-code" }));
			await h[0].handler(eCtx(a, { email: "alice@example.com", inviteCode: "valid-code" }));
			// Second call overwrites — maps have set semantics
			expect(__pendingInvites.get("alice@example.com")?.invitationId).toBe("inv-B");
		});
	});
	...
	describe("before — OAuth", () => {
		it("INVITE_REQUIRED when no cookie", async () => {
			const a = mkAdapter();
			await expect(createBeforeHooks(OPTS)[1].handler(oCtx(a))).rejects.toThrow(ERROR_CODES.INVITE_REQUIRED);
		});
		...
		it("SEC-1: concurrent OAuth with different codes = separate entries", async () => {
			const a = mkAdapter();
			a.findOne.mockResolvedValueOnce(inv({ id: "A" })).mockResolvedValueOnce(inv({ id: "B" }));
			const h = createBeforeHooks(OPTS);
		...
		it("no OAuth hook when interceptOauth=false", () => {
			expect(createBeforeHooks({ ...OPTS, interceptOauth: false })).toHaveLength(1);
		});

		it("OAuth does NOT enforce email binding (design gap)", async () => {
			// Invitation bound to alice@, but OAuth has no email to check — passes
			const a = mkAdapter(); a.findOne.mockResolvedValue(inv({ email: "alice@specific.com" }));
			await expect(createBeforeHooks(OPTS)[1].handler(oCtx(a, "valid-code"))).resolves.not.toThrow();
		});
	...
	describe("after — consuming invitations", () => {
		it("consumes by email key and updates DB", async () => {
			const a = mkAdapter(); a.update.mockResolvedValue({});
			__pendingInvites.set("alice@example.com", { invitationId: "inv-1", createdAt: Date.now() });
			await createAfterHooks(OPTS)[0].handler(aCtx(a, { id: "u1", email: "Alice@Example.com" }));
			expect(a.update).toHaveBeenCalledWith(expect.objectContaining({
				model: "invitation", where: [{ field: "id", value: "inv-1" }],
			}));
			expect(__pendingInvites.has("alice@example.com")).toBe(false);
		});
		...
		it("consumes by __code: key for OAuth", async () => {
			const a = mkAdapter(); a.update.mockResolvedValue({});
			__pendingInvites.set("__code:oc", { invitationId: "inv-o", createdAt: Date.now() });
		...
		it("prefers email key over __code: key", async () => {
			const a = mkAdapter(); a.update.mockResolvedValue({});
			__pendingInvites.set("alice@example.com", { invitationId: "by-email", createdAt: Date.now() });
			__pendingInvites.set("__code:c", { invitationId: "by-code", createdAt: Date.now() });
			await createAfterHooks(OPTS)[0].handler(aCtx(a, { id: "u1", email: "alice@example.com" }, "c"));
			expect(a.update).toHaveBeenCalledWith(expect.objectContaining({
		...
		it("no-op when no pending entry (TTL expired between hooks)", async () => {
			const a = mkAdapter();
			await createAfterHooks(OPTS)[0].handler(aCtx(a, { id: "u1", email: "a@b.com" }));
			expect(a.update).not.toHaveBeenCalled();
		});

		it("catches adapter.update failure and logs", async () => {
			const a = mkAdapter(); a.update.mockRejectedValue(new Error("boom"));
			__pendingInvites.set("a@b.com", { invitationId: "inv-1", createdAt: Date.now() });
			const ctx = aCtx(a, { id: "u1", email: "a@b.com" });
			await expect(createAfterHooks(OPTS)[0].handler(ctx)).resolves.not.toThrow();
			expect(ctx.context.logger.error).toHaveBeenCalled();
		});

		it("reads from ctx.context.returned.user when newUser absent", async () => {
			const a = mkAdapter(); a.update.mockResolvedValue({});
			__pendingInvites.set("bob@t.com", { invitationId: "inv-r", createdAt: Date.now() });
			const ctx = {
				path: "/sign-up/email", body: {}, headers: new Headers(),
				context: { adapter: a, returned: { user: { id: "ur", email: "bob@t.com" } }, logger: { error: vi.fn() } },
			};
			await createAfterHooks(OPTS)[0].handler(ctx);
			expect(a.update).toHaveBeenCalled();
		});
		...
		it("no-op when both newUser and returned are absent", async () => {
			const a = mkAdapter();
			const ctx = { path: "/sign-up/email", body: {}, headers: new Headers(), context: { adapter: a } };
			await expect(createAfterHooks(OPTS)[0].handler(ctx)).resolves.not.toThrow();
			expect(a.update).not.toHaveBeenCalled();
		});
		...
		it("no-op when user.email is null (no email key, no code key)", async () => {
			const a = mkAdapter();
			await createAfterHooks(OPTS)[0].handler(aCtx(a, { id: "u1", email: null as any }));
			expect(a.update).not.toHaveBeenCalled();
		});

		it("still works without logger (optional chaining)", async () => {
			const a = mkAdapter(); a.update.mockRejectedValue(new Error("fail"));
			__pendingInvites.set("a@b.com", { invitationId: "inv-1", createdAt: Date.now() });
			const ctx = {
				path: "/sign-up/email", body: {}, headers: new Headers(),
	...
	describe("state corruption", () => {
		it("enabled toggling mid-flight via async function", async () => {
			...
			await expect(h[0].handler(eCtx(a, { email: "a@b.com" }))).rejects.toThrow(ERROR_CODES.INVITE_REQUIRED);
			await expect(h[0].handler(eCtx(a, { email: "b@b.com" }))).resolves.not.toThrow();
		});

		it("enabled function that throws is not caught — propagates", async () => {
			const a = mkAdapter();
			const h = createBeforeHooks({ ...OPTS, enabled: () => { throw new Error("config error"); } });
			await expect(h[0].handler(eCtx(a, { email: "a@b.com" }))).rejects.toThrow("config error");
		});

		it("adapter.findOne throwing propagates (not silently swallowed)", async () => {
			const a = mkAdapter(); a.findOne.mockRejectedValue(new Error("DB timeout"));
			await expect(createBeforeHooks(OPTS)[0].handler(eCtx(a, { email: "a@b.com", inviteCode: "x" })))

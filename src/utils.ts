...
export function computeInvitationStatus(
	invitation: Pick<Invitation, "revokedAt" | "usedAt" | "expiresAt">,
): InvitationStatus {
...
/**
 * Check if an invitation is valid for use (not used, not revoked, not expired).
 */
export function isInvitationValid(
	invitation: Pick<Invitation, "usedAt" | "revokedAt" | "expiresAt">,
): boolean {
